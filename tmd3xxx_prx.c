...
static int tmd3xxx_init(struct sensor_api_t *s);
static int tmd3xxx_activate(struct sensor_api_t *s, int enable);
static int tmd3xxx_set_delay(struct sensor_api_t *s, int64_t ns);
static void tmd3xxx_close(struct sensor_api_t *s);
static void *tmd3xxx_read(void *arg);
...
static struct sensor_desc tmd3xxx = {
	.sensor = {
		.name = "TMD/TSL 3XXX Proximity",
	...
	.api = {
		.init = tmd3xxx_init,
		.activate = tmd3xxx_activate,
		.set_delay = tmd3xxx_set_delay,
		.close = tmd3xxx_close
	},
	.name = "taos_proximity",
};
...
static int tmd3xxx_init(struct sensor_api_t *s)
{
	...
	close(fd);
	sensors_select_init(&d->select_worker, tmd3xxx_read, s, -1);
...
static void *tmd3xxx_read(void *arg)
{
	...
	for (i = 0; i < (bytes / (int)sizeof(struct input_event)); i++) {
		...
		case EV_SYN:
			...
			data.version = tmd3xxx.sensor.version;
			data.sensor = tmd3xxx.sensor.handle;
			data.type = tmd3xxx.sensor.type;
...
list_constructor(tmd3xxx_init_driver);
void tmd3xxx_init_driver()
{
	(void)sensors_list_register(&tmd3xxx.sensor, &tmd3xxx.api);
}
